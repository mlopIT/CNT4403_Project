...
def generate_unique_filename(base_name="md5hash_raw.txt"):
    """
    Generates a unique filename by appending a number if the file already exists.
    e.g., md5hash_raw.txt, md5hash_raw1.txt, md5hash_raw2.txt, etc.
    """
...
def hash_passwords_in_file_md5_raw(filename):
    """
    Reads passwords from a file and hashes them using RAW MD5
    (no salting or key stretching) and writes the hash to a unique output file
    with each hash on a new line.

    """
    # NOTE ON MD5: MD5 is considered Cryptographically Broken and is NOT suitable
    # for secure password hashing. This script is for demonstration/legacy purposes only.
    try:
        # Determine the unique output filename
        output_file = generate_unique_filename(base_name="md5hash_raw.txt")
        ...
        with open(filename, 'r', encoding="utf-8") as fin:
            for line in fin:
                ...
                # !!! SECURITY WARNING: MD5 is cryptographically broken and unsafe for passwords.
                hasher = hashlib.md5()
        ...
        print(f"Hashed passwords (raw MD5) written to: {output_file}")
...
def introductory_prompt():
    ...
    print("==================================================================")
    print("      Password File Hasher (using RAW MD5)")
    print("------------------------------------------------------------------")
    ...
    print("It will then hash the passwords using the raw MD5 algorithm.")
    print("==================================================================")
...
if __name__ == "__main__":
    ...
    if chosen_file_path:
        hash_passwords_in_file_md5_raw(chosen_file_path)
    else:
